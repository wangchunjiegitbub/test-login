...
{"status":0}
 */
var server = "http://192.168.1.104:8080/dmh/update"; //获取升级描述文件服务器地址
...
function update() {
	...
	console.log("update111222");

	mui.getJSON(server,'data=' + JSON.stringify(data), function(data) {
		if(data.status) {
			plus.nativeUI.confirm(data.note, function(event) {
				if(0 == event.index) {
					plus.runtime.openURL(data.url);
				}
			}, data.title, ["立即更新122", "取　　消"]);
		}
	});
